...
export default function SubpageHeader({title, description}: SubpageHeaderProps) {
  return (
      ...
      <BackgroundGradientAnimation
        ...
        <div className="absolute z-10 inset-0 flex items-center justify-start px-4">
          <div className="max-w-4xl mx-auto flex flex-col items-start space-y-4">
            <h1 className="text-4xl md:text-5xl font-semibold text-left text-emerald-900">{title}</h1>
            {description && <p className="text-lg md:text-xl text-emerald-800/80 font-lato max-w-3xl">{description}</p>}
          </div>
        </div>
      </BackgroundGradientAnimation>
    </div>
